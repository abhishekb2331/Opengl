...
void drawChain(void);
void drawHeadlight();
void relNormalkeys(unsigned char key,int x,int y); 
...
void relNormalkeys(unsigned char key,int x,int y)
{
	if(key=='a')
	{
		r_flag=0;r_ang=0;left_turn=false;
	}
	if(key=='d')
	{
		r_flag=0;r_ang=0;right_turn=false;
	}
	if(key=='w')
	{
		deltaMove = 0.0;
...
void drawSnowman()
{
	/*	glColor3f(0.85,0.85,0.1); 
		glPushMatrix();
		glTranslatef(0.0, 16.0, 25.75);
		glRotatef(fossil_angle,1,0,0);
		glutSolidCylinder(0.75,0.15, 20, 20);
		glPopMatrix();*/
	glRotatef(cangle,0,1,0);
	glColor3f(1.0,1.0,1.0);
	glutSolidCylinder(1.0,0.1,20,20);
...
void update(void) 
{
	...
	if(tflag1==1)
	{
		tflag2=0;tim-=1;tflag1=0;totalt++;
		if(totalt>tim/2+1)
		{mlight=0;
			blight=1;
		}
	...
	if(1 && gameover!=1)
	{
		...
		if(temp>360)
			temp-=360;
		per_x=-sin(DEG2RAD(temp));
		per_y=cos(DEG2RAD(temp));
		if(deltaMove==1)
		{
			if(acc<1)
				acc+=0.0005;
		...
		{ // update camera position
			if(box_y<0 || box_y>_terrain->width())
				gameover=1;
			box_y += deltaMove * ly * (0.7)+acc*ly;
			if(box_x<0 || box_x>_terrain->length())
				gameover=1;
			box_x += deltaMove * lx * (0.7)+acc*lx;
		}
		else
		{
			box_x+=0.7*lx*acc;
	...
	for(int i=0;i<18;i++)
	{
		if(place_x[i]>=box_x-0.7f && place_x[i]<=box_x+0.7f)
			if(place_y[i]>=box_y-0.7f && place_y[i]<=box_y+0.7f)
			{
				place_x[i]=rand()%159;
				place_y[i]=rand()%159;
				score++;
...
void renderScene(void) 
{
	if(gameover!=1)
	{
		...
		t+=0.3;
		u+=0.45;
		if(t>_terrain->length())
			t=1;
		if(u>_terrain->length())
			u=1;
		glPopMatrix();
		...
		printf("%f %f %d %d\n",init_speed,max_speed,init_time,tim);
		for(i = 0 ; i < 18; i++)
			//              for(j = 0; j < 8; j++) {
		{       glPushMatrix();
			//                      float place_x=rand()%59;
			//                      float place_y=rand()%59;
			//              float h=_terrain->getHeight((i*17.5+200.0f)*59/400,(27.5*j +200.0f)*59/400);
			float h=heightAt(_terrain,place_x[i], place_y[i]);
			glTranslatef(place_x[i], place_y[i], h+1);
			glRotatef(90,1,0,0);
			cangle+=0.1f;
			drawSnowman();
			glPopMatrix();
		...
		z_co=heightAt(_terrain,box_x,box_y);
		float z2=heightAt(_terrain,box_x+lx*4.0,box_y+ly*4.0);
		h1=z2;h2=h1;
		if(jump==1)
		{
			h1=prev_ht-0.1;
		...
		if(prev_ht-h2>0.1)
		{
			...
			jump=0;
		prev_ht=h1;
		var_angle=(atan((z2-z_co)/4)*100)/PI;
		...
		glPushMatrix();
		glTranslatef(box_x+4*per_x,box_y+4*per_y,z_co+2.5f);
		glColor3f(0.0,0.0,1.0);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		glBegin(GL_QUADS);
		glVertex3f(-per_x/4,-per_y/4,-2);
		glVertex3f(per_x/4,per_y/4,-2);
		glVertex3f(per_x/4,+per_y/4,(float)((1.4*tim)/init_time));
		glVertex3f(-per_x/4,-per_y/4,(float)((1.4*tim)/init_time));
		glEnd();
		glPopMatrix();
		glPushMatrix();
...
//		printf("ratio:: %f\n",((2.0*tim)/init_time));
		glVertex3f(-per_x/4,-per_y/4,(float)((1.4*tim)/init_time));
		glVertex3f(+per_x/4,per_y/4,(float)((1.4*tim)/init_time));
		glVertex3f(+per_x/4,per_y/4,1.4);
		glVertex3f(-per_x/4,-per_y/4,1.4);
		glEnd();
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(box_x-4*per_x,box_y-4*per_y,z_co+0.5f);
		glColor3f(0.0,0.0,1.0);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		glBegin(GL_QUADS);
		glVertex3f(-per_x/4,-per_y/4,0);
		glVertex3f(per_x/4,per_y/4,0);
		glVertex3f(per_x/4,+per_y/4,(float)((3.4*init_speed)/max_speed));
		glVertex3f(-per_x/4,-per_y/4,(float)((3.4*init_speed)/max_speed));
		glEnd();
		glPopMatrix();
		glPushMatrix();
...
//		printf("ratio:: %f\n",((2.0*tim)/init_time));
		glVertex3f(-per_x/4,-per_y/4,(float)((3.4*init_speed)/max_speed));
		glVertex3f(+per_x/4,per_y/4,(float)((3.4*init_speed)/max_speed));
		glVertex3f(+per_x/4,per_y/4,3.4);
		glVertex3f(-per_x/4,-per_y/4,3.4);
		glEnd();
		glPopMatrix();
		...
		//	glRasterPos3f(_terrain->length()/2, _terrain->width()/2-7.0f, 3.0 +z_co);
		glScalef(5.0f,5.0f,5.0f);
		int k;
		GLvoid *font_style=GLUT_BITMAP_HELVETICA_18;
		char btime[100];
		sprintf(btime,"Time Left:%02d:%02d Score:%d",tim/60,tim%60,score);
		for(k=0;btime[k]!='\0';k++)
		...
		/*	if(gameover==1)
			...
			glRasterPos3f(box_x -6*lx-0.1, box_y - 6*ly, 3.0 +z_co);
			char ttt[]={"GAME OVER"};
			for(k=0;ttt[k]!='\0';k++)
			{
	...
	/*	else if(start_game==1)
		...
		glPushMatrix();
		glColor3f(1.0,0.3,1.0);
	//		float rrr=box_x,ppp=box_y;
	//glRasterPos3f(_terrain->length()/2, _terrain->width()/2-7.0f, 3.0 +z_co);
	glRasterPos3f(box_x -1*lx, box_y - 1*ly,z_co+3);
	char ttt[]={"Press Enter To Start"};
	for(int k=0;ttt[k]!='\0';k++)
	{
	glutBitmapCharacter(font_style,ttt[k]);
	}
...
else if(gameover==1)
{
	...
	//glRasterPos3f(_terrain->length()/2, _terrain->width()/2-7.0f, 3.0 +z_co);
	glRasterPos3f(box_x -1*lx, box_y - 1*ly,z_co+3);
	char ttt[]={"GAME OVER, Please Press Esc To Quit"};
	for(int k=0;ttt[k]!='\0';k++)
	{
		glutBitmapCharacter(font_style,ttt[k]);
	}
...
void drawHeadlight()
{
	...
	glLightfv(GL_LIGHT1, GL_SPECULAR, light1_specular); 
	GLfloat _light1_position[4]; 
	_light1_position[0] = 0.0; 
	_light1_position[1] = 0.0;
	_light1_position[2] = 1.0;
	_light1_position[3] = 1.0;
	GLfloat _spotlight_position[4];
	_spotlight_position[0] = 1.0;
	...
	glEnable(GL_LIGHT1);
	//cout << "Headlight\n";
	glLightfv(GL_LIGHT1, GL_POSITION, _light1_position);
...
int main(int argc, char **argv) 
{
	...
	glutKeyboardUpFunc(relNormalkeys);
